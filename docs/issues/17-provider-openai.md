...
2. `translateBatch`: `buildPrompt(req)` → chat completions request
   ...
   `signal`. Response text → `parseProviderResponse` → `TranslationResult`; map
...
5. Tests (nock or the SDK's fetch injection; no live API):
   - request assertion: URL (default and overridden baseUrl), auth header uses the init
     key, body contains `response_format: json_object`, temperature 0, both messages;
   - happy path parses translations + usage;
   - 429/500 → retryable ProviderError; 401 → non-retryable;
   - abort via signal → rejects promptly (fake timers);
   - JSON-with-fence content handled via shared parser (delegation test).
6. Provider must not read env directly (credentials arrive via `ProviderInit` — test by
   constructing with explicit init while env unset).

## Acceptance Criteria

- [ ] SDK retries disabled; exactly one HTTP call per `translateBatch` (mock counts).
- [ ] Error mapping table above fully tested.
- [ ] `baseUrl` override hits the alternate host (mock asserts).
- [ ] No env access inside the module; key never in any thrown message (test greps).
- [ ] Works end-to-end with Issue 16 runner against the mock (one integration case).
